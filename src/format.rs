...
/// The ephemeral key bytes in a ledger-k256 stanza.
///
/// The bytes contain a compressed SEC-1 encoding of a valid point.
#[derive(Debug)]
pub(crate) struct EphemeralKeyBytes(k256::EncodedPoint);
...
impl EphemeralKeyBytes {
    fn from_bytes(bytes: [u8; EPK_BYTES]) -> Option<Self> {
        ...
        if encoded.is_compressed()
            && k256::PublicKey::from_encoded_point(&encoded)
                .is_some()
                .into()
        {
    ...
    fn from_public_key(epk: &k256::PublicKey) -> Self {
    ...
    pub(crate) fn decompress(&self) -> k256::EncodedPoint {
        // EphemeralKeyBytes is a valid compressed encoding by construction.
        let p = k256::PublicKey::from_encoded_point(&self.0).unwrap();
